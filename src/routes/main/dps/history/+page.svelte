<script lang="ts">
  ...
  async function confirmDeleteSelected() {
    ...
    try {
      ...
      if (res.status === "ok") {
        ...
        errorMsg = $t("deleteFailed", { error: String(res.error) });
      }
    } catch (e) {
  ...
  async function loadEncounters(p: number = page) {
    ...
    try {
      ...
      const filterPayload: EncounterFiltersDto = {
        ...
        playerName: null,
        encounterNames: selectedEncounters.length > 0 ? selectedEncounters : null,
  ...
  function removeEncounterFilter(encounterName: string) {
    selectedEncounters = selectedEncounters.filter((name) => name !== encounterName);
    loadEncounters(0);
  }
        ...
        {#if showFavoritesOnly}
          ...
          >
            ...
            <button
              ...
              aria-label={$t("removeFavoriteFilter")}
            >
              ✕
            </button>
          </span>
        {/if}
        {#each selectedBosses as boss (boss)}
          <span
          ...
          >
            <span class="text-muted-foreground/70">{$t("bossPrefix")}</span>
            {boss}
            <button
              onclick={() => removeBossFilter(boss)}
              class="text-muted-foreground/70 hover:text-destructive transition-colors"
              aria-label={$t("removeBossFilter", { boss })}
            >
              ✕
            </button>
          </span>
        {/each}
        {#each selectedPlayerNames as player (player)}
          <span
            class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] bg-popover text-muted-foreground leading-tight border border-border/60"
          >
            <span class="text-muted-foreground/70">{$t("playerPrefix")}</span>
            {player}
            <button
              onclick={() => removePlayerNameFilter(player)}
              class="text-muted-foreground/70 hover:text-destructive transition-colors"
              aria-label={$t("removePlayerFilter", { player })}
            >
              ✕
            </button>
          </span>
        {/each}
        {#each selectedEncounters as encounter (encounter)}
          <span
            class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] bg-popover text-muted-foreground leading-tight border border-border/60"
          >
            <span class="text-muted-foreground/70">{$t("scenePrefix")}</span>
            {encounter}
            <button
              onclick={() => removeEncounterFilter(encounter)}
              class="text-muted-foreground/70 hover:text-destructive transition-colors"
              aria-label={$t("removeSceneFilter", { scene: encounter })}
            >
              ✕
            </button>
          </span>
  ...
  <div class="overflow-x-auto rounded border border-border/60 bg-card/30 relative">
    <div class="absolute top-2 right-3 z-10">
      <button
        ...
        aria-label={$t("refreshCombatList")}
      >
        <svg
            ...
            <td class="px-3 py-2 text-sm text-muted-foreground">
              <button
                ...
                class="flex items-center justify-center w-5 h-5 rounded border transition-colors {selectedIds.has(
                  ...
                  ? 'bg-primary border-primary'
                  : 'border-border hover:border-primary/50'}"
                aria-label={selectedIds.has(enc.id) ? $t("deselect") : $t("select")}
              >
              ...
              <div class="space-y-1">
                <div>
                  {#if enc.sceneName}
                    ...
                    >
                  {:else}
                    <span class="text-muted-foreground text-xs opacity-70">{$t("noScene")}</span>
                  {/if}
                </div>
                <div>
            ...
            <td class="px-3 py-2 text-sm text-muted-foreground">
              {#if enc.players.length > 0}
                ...
                <div class="flex gap-1 items-center">
                  {#each sortedPlayers.slice(0, 8) as player (player)}
                    <img
    ...
    <div class="flex items-center gap-3 text-sm text-muted-foreground">
      ...
      <span>
        {$t("showingRows", {
          ...
          end: String(Math.min((page + 1) * pageSize, totalCount)),
  ...
  >
    ...
    >
      <div class="flex items-center gap-2 text-sm">
        ...
        <span class="text-muted-foreground">{$t("recordsSelected")}</span>
      </div>
  ...
  >
    ...
    >
      <div class="flex items-center gap-3 mb-4">
        ...
        <div>
          <h3 id="delete-modal-title" class="text-lg font-semibold text-foreground">
            {$t("deleteRecordsTitle", { count: String(selectedIds.size) })}
          </h3>
      ...
      <p class="text-sm text-muted-foreground mb-6">
        {$t("deleteRecordsConfirm", { count: String(selectedIds.size) })}
      </p>
